...
class FrCompanyRetrieveParams(BaseModel):
    ...
    id: int = Field(..., description="Internal company ID (from financial_reports_companies_search results)")
...
class FrFilingRetrieveParams(BaseModel):
    ...
    id: int = Field(..., description="Filing ID (from financial_reports_filings_search results)")
...
class FrNextAnnualReportParams(BaseModel):
    ...
    id: int = Field(..., description="Company ID (from financial_reports_companies_search results)")
...
class FrReferenceDataParams(BaseModel):
    ...
    resource: Literal[
        ...
        description=(
            ...
            "Use filing_types/filing_categories to discover valid filter values for financial_reports_filings_search."
        ),
    )
...
async def financial_reports_companies_search(params: FrCompaniesSearchParams, *, api_key: str) -> Result:
    ...
    Use the company ID with financial_reports_filings_search(company=id) to find filings,
    financial_reports_next_annual_report(id=id) for report predictions, or
    financial_reports_company_retrieve(id=id) for the full profile.
    Use financial_reports_isic_browse to discover valid ISIC codes for sector/industry filters.
    """
...
async def financial_reports_company_retrieve(params: FrCompanyRetrieveParams, *, api_key: str) -> Result:
    ...
    Use financial_reports_filings_search(company=id) to find this company's filings.
    Use financial_reports_isin_lookup(company=id) for ISIN/FIGI cross-references.
    """
...
async def financial_reports_filings_search(params: FrFilingsSearchParams, *, api_key: str) -> Result:
    ...
    Use the filing ID with financial_reports_filing_retrieve(id=id) for full details,
    financial_reports_filing_markdown(id=id) to read the document content, or
    financial_reports_filing_history(id=id) for the audit trail.
    Discover valid filing type codes via financial_reports_reference_data(resource='filing_types').
    """
    kwargs = _strip_none(params.model_dump())
...
async def financial_reports_filing_retrieve(params: FrFilingRetrieveParams, *, api_key: str) -> Result:
    ...
    Use financial_reports_filing_markdown(id=id) to read the actual document content.
    Use financial_reports_filing_history(id=id) for the audit trail of changes to this filing.
    """
...
@connector(tags=["financial_reports"], result_type="text")
async def financial_reports_filing_markdown(params: FrFilingMarkdownParams, *, api_key: str) -> Result:
    ...
    Use financial_reports_filing_retrieve(id=id) first to check if markdown_url is available.
    """
...
async def financial_reports_filing_history(params: FrFilingHistoryParams, *, api_key: str) -> Result:
    ...
    Use financial_reports_filing_retrieve(id=id) first to get the current state of the filing.
    """
...
async def financial_reports_next_annual_report(params: FrNextAnnualReportParams, *, api_key: str) -> Result:
    ...
    historical release pattern. Use financial_reports_companies_search to find company IDs.
    """
...
async def financial_reports_isic_browse(params: FrIsicBrowseParams, *, api_key: str) -> Result:
    """Browse ISIC industry classifications to find valid filter codes.

    Returns code/name pairs at the chosen hierarchy level.
    Use the codes in financial_reports_companies_search: sector (sections), industry_group
...
async def financial_reports_isin_lookup(params: FrIsinLookupParams, *, api_key: str) -> Result:
    ...
    Use financial_reports_companies_search to find company IDs, then financial_reports_isin_lookup(company=id)
...
async def financial_reports_reference_data(params: FrReferenceDataParams, *, api_key: str) -> Result:
    ...
    - filing_types: type codes for financial_reports_filings_search(type=..., types=...)
    - countries: alpha_2 codes for financial_reports_filings_search(countries=...)
      or financial_reports_companies_search(countries=...)
    - filing_categories: category IDs for financial_reports_filings_search(category=..., categories=...)
    - languages: ISO 639-1 codes for financial_reports_filings_search(language=..., languages=...)
    - sources: source IDs for financial_reports_filings_search(source=..., sources=...)
    """
    kwargs: dict[str, Any] = {}
    if params.page is not None:
        kwargs["page"] = params.page
