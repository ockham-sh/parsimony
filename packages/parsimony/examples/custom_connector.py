...
Expected output:
    ...
    followed by proof that it lives alongside the FRED fetch connector in the same bundle.
...
async def main() -> None:
    ...
    fred_bundle = Connectors([fred]).bind_deps(api_key=api_key)
    bundle = fred_bundle + Connectors([my_data_source])
