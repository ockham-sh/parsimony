...
class TestFmpQuotes:
    ...
    async def test_uses_batch_quote(self) -> None:
        ...
        with _patch_http(_make_response(data)) as m:
            await _call(fmp_quotes, symbols="AAPL,MSFT")
...
class TestFmpPrices:
    ...
    async def test_intraday_path(self) -> None:
        ...
        data = [
            {"date": "2024-01-02 10:00:00", "open": 180.0, "high": 180.5, "low": 179.5, "close": 180.2, "volume": 1e5}
        ]
        with _patch_http(_make_response(data)) as m:
...
class TestFmpFinancialStatements:
    ...
    async def test_income_statement_no_symbol_in_path(self) -> None:
        ...
        data = [
            {
                ...
                "revenue": 3.91e11,
                "costOfRevenue": 2.14e11,
                "grossProfit": 1.77e11,
                "operatingExpenses": 5.5e10,
                "operatingIncome": 1.22e11,
                "ebitda": 1.3e11,
                "netIncome": 9.7e10,
                "eps": 6.42,
                "epsDiluted": 6.32,
        ...
        with _patch_http(_make_response(data)) as m:
            await _call(fmp_income_statements, symbol="AAPL", period="annual", limit=1)
        ...
        data = [
            {
                ...
                "totalDebt": 1.11e11,
                "netDebt": 7.6e10,
                "cashAndCashEquivalents": 3.5e10,
            }
        ]
        with _patch_http(_make_response(data)) as m:
            await _call(fmp_balance_sheet_statements, symbol="AAPL")
        path = m.call_args.args[1]
    ...
    async def test_cash_flow_no_symbol_in_path(self) -> None:
        ...
        data = [
            {
                ...
                "reportedCurrency": "USD",
                "netIncome": 9.7e10,
                "operatingCashFlow": 1.18e11,
                "capitalExpenditure": -1.1e10,
                "freeCashFlow": 1.07e11,
        ...
        with _patch_http(_make_response(data)) as m:
            await _call(fmp_cash_flow_statements, symbol="AAPL")
        path = m.call_args.args[1]
...
class TestFmpNews:
    ...
    async def test_stock_news_path(self) -> None:
        ...
        data = [
            {
                "symbol": "AAPL",
                "publishedDate": "2024-10-31T14:00:00",
                "title": "Q4",
                "text": "Apple...",
                "url": "https://x.com",
                "site": "Reuters",
        ...
        data = [
            {
                ...
                "investorsHoldingChange": 50,
                "numberOf13Fshares": 1.5e10,
                "numberOf13FsharesChange": 2e8,
                "totalInvested": 2.7e12,
                "totalInvestedChange": 1e11,
                "ownershipPercent": 0.62,
                "ownershipPercentChange": 0.01,
                "newPositions": 120,
                "closedPositions": 80,
                "increasedPositions": 2500,
                "reducedPositions": 1800,
                "putCallRatio": 0.75,
            }
        ]
        with _patch_http(_make_response(data)) as m:
...
class TestFmpEarningsTranscript:
    ...
    async def test_path(self) -> None:
        ...
        data = [{"symbol": "AAPL", "year": 2024, "period": "Q4", "date": "2024-10-31", "content": "Good afternoon..."}]
        with _patch_http(_make_response(data)) as m:
...
class TestFmpMarketMovers:
    ...
    async def test_gainers(self) -> None:
        ...
        data = [
            {"symbol": "X", "name": "X", "price": 50.0, "change": 10.0, "changesPercentage": 25.0, "exchange": "NASDAQ"}
        ]
        with _patch_http(_make_response(data)) as m:
...
class TestFmpErrorHandling:
    ...
    async def test_401_friendly_message(self) -> None:
        ...
        with (
            _patch_http(_make_response({}, status_code=401)),
            pytest.raises(UnauthorizedError, match="Invalid or missing FMP API key"),
        ):
    ...
    async def test_402_plan_message(self) -> None:
        ...
        with (
            _patch_http(_make_response({}, status_code=402)),
            pytest.raises(PaymentRequiredError, match="not eligible"),
        ):
            await _call(fmp_company_profile, symbol="AAPL")
    ...
    async def test_500_generic_message(self) -> None:
        ...
        with _patch_http(_make_response({}, status_code=500)), pytest.raises(ProviderError, match="FMP API error 500"):
            await _call(fmp_company_profile, symbol="AAPL")
...
class TestFmpScreener:
    ...
    def _metrics(self, sym):
        return [{"symbol": sym, "enterpriseValueTTM": 2.8e12, "returnOnEquityTTM": 0.15, "freeCashFlowYieldTTM": 0.035}]

    def _ratios(self, sym):
        return [
            {"symbol": sym, "priceToEarningsRatioTTM": 28.5, "grossProfitMarginTTM": 0.45, "dividendYieldTTM": 0.005}
        ]
    ...
    async def test_basic_screener(self) -> None:
        ...
        with patch("parsimony.transport.http.HttpClient.request", side_effect=self._mock_request):
    ...
    async def test_skips_enrichment_when_not_needed(self) -> None:
        ...
        with patch("parsimony.transport.http.HttpClient.request", side_effect=tracking_mock):
    ...
    async def test_invalid_where_clause(self) -> None:
        ...
        with (
            patch("parsimony.transport.http.HttpClient.request", side_effect=self._mock_request),
            pytest.raises(ValueError, match="Invalid where_clause"),
        ):
...
class TestLiveFmpConnectors:
    ...
    async def test_prices_daily(self) -> None:
        ...
        result = await _call_live(
            fmp_prices, symbol="AAPL", frequency="daily", **{"from": "2024-01-02", "to": "2024-01-05"}
        )
        assert len(result.df) >= 1
    ...
    async def test_prices_dividend_adjusted(self) -> None:
        ...
        result = await _call_live(
            fmp_prices, symbol="AAPL", frequency="dividend_adjusted", **{"from": "2024-01-02", "to": "2024-01-05"}
        )
        assert len(result.df) >= 1
