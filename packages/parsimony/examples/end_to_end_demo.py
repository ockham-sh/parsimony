...
This script is self-contained (in-memory catalog, no API keys). Requires
``parsimony-core[standard]`` for the standard catalog backend (FAISS + BM25 +
sentence-transformers). It shows:
...
4. **Catalog** — :meth:`~parsimony.Catalog.index_result` reads ``namespace`` from
   the KEY column.
...
async def _section_catalog_and_callbacks() -> None:
    catalog = Catalog(DEMO_NAMESPACE)

    typed_conn: Connector = demo_fetch_typed

    print("\n--- 4) Manual index_result(table) ---\n")
    table = await typed_conn(label="demo")
    assert isinstance(table, SemanticTableResult)
    idx = await catalog.index_result(table)
    print(f"  IndexResult: total={idx.total}, indexed={idx.indexed}, skipped={idx.skipped}")

    listed, total = await catalog.list(namespace=DEMO_NAMESPACE, limit=10)
    print(f"  list: total={total}, codes={[e.code for e in listed]}")

    print("\n--- 5) dry_run=True (dedupe preview; no writes) ---\n")
