...
class TestConnectorsToLlm:
    ...
    def test_empty_collection_is_empty_string(self) -> None:
        # Pure serialization: nothing to serialize → empty string. Headers,
        # narrative, and "no connectors" framing are the host's concern.
        assert Connectors([]).to_llm() == ""

    def test_each_connector_serialized(self) -> None:
        coll = Connectors([fred_search, bare_connector])
        text = coll.to_llm()
        assert "### fred_search" in text
        assert "### bare_connector" in text

    def test_no_framework_authored_header(self) -> None:
        # The framework must not inject usage prose, binding names, or section
        # headings — that's prompt authoring, which lives in the host.
        text = Connectors([fred_search]).to_llm()
        assert "Data connectors" not in text
        assert "client[" not in text
        assert "## Connectors" not in text
        assert "## Tools" not in text
        assert "ONLY use connectors" not in text

    def test_concatenation_separates_with_blank_line(self) -> None:
        text = Connectors([fred_search, bare_connector]).to_llm()
        assert "\n\n### bare_connector" in text
