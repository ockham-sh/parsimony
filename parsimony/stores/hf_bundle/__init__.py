...
Internal package layout (no public API change — import
``HFBundleCatalogStore`` and ``LoadedNamespace`` from this module as before):

- :mod:`.cache_layout` — filesystem helpers (`_bundle_dir`, scan, cleanup).
- :mod:`.hf_download` — HuggingFace Hub I/O (HEAD check, snapshot download).
- :mod:`.integrity` — on-disk parse, sha256 + identity verify, FAISS search,
  plus :class:`LoadedNamespace`.
"""
