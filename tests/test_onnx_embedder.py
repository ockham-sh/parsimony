...
def test_onnx_embed_texts_roundtrip(tmp_path: Path) -> None:
    emb = OnnxEmbedder(cache_dir=tmp_path, quantize=True)
...
def test_onnx_query_matches_related_doc_better(tmp_path: Path) -> None:
    ...
    query = emb.embed_query("euro area 10Y bond yield")
...
def test_onnx_and_sentence_transformer_agree_on_ordering(tmp_path: Path) -> None:
    ...
    onnx_vecs = onnx.embed_texts(docs)
    st_vecs = st.embed_texts(docs)

    def dot(a: list[float], b: list[float]) -> float:
        return sum(x * y for x, y in zip(a, b, strict=True))

    query = "10 year bund yield"
    onnx_q = onnx.embed_query(query)
    st_q = st.embed_query(query)
...
def test_onnx_catalog_end_to_end(tmp_path: Path) -> None:
    ...
    cat.save(bundle_dir)
    loaded = Catalog.load(bundle_dir)
