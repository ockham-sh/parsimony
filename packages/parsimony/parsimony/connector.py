...
def connector(
    ...
    **Canonical usage** — infer metadata from the function; add ``output`` when needed::
        ...
        async def fred(params: FredFetchParams, *, api_key: str) -> Result:
...
class Connectors:
    ...
    Lookup by name: ``connectors["fred"]`` or ``connectors.get("fred")``.
    ...
    def to_llm(self) -> str:
        """Return the LLM-facing serialization of every connector in this collection.

        Pure serialization: each connector's :meth:`Connector.to_llm` rendered
        and joined with blank lines. No header, no usage prose, no binding
        name. The host application owns prompt framing — including any "how to
        call these" narrative, the binding name, and section headings — and
        composes this output into its own prompt structure (system prompt,
        per-turn context block, MCP server instructions, etc.).
        """
        if not self._items:
            return ""
        return "\n\n".join(c.to_llm() for c in self._items) + "\n"
