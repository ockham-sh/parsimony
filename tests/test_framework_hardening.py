...
# ---------------------------------------------------------------------------
# Non-transforming wrap: the framework never coerces or validates payloads
# ---------------------------------------------------------------------------


def test_connector_payload_passes_through_untouched() -> None:
    """Declaring an OutputSpec must not rename, coerce, or reject the returned frame."""
    ...
    output = OutputSpec(
        columns=[
            ...
            Column(name="declared_but_absent", role=ColumnRole.DATA),
        ]
    )
    ...
    assert result.data["price"].tolist() == ["n/a"]
    assert [c.name for c in result.columns] == ["ts", "price", "declared_but_absent"]
