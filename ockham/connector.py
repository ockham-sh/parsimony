...
Pass ``output=`` when the connector should apply a tabular :class:`~ockham.result.OutputConfig`.
...
(no TITLE/METADATA). Results are persisted via :class:`~ockham.data_store.DataStore`.
...
from ockham.catalog.models import normalize_code
from ockham.result import ColumnRole, OutputConfig, Provenance, Result
...
def loader(
    ...
    **Validation:** ``output`` must have no TITLE or METADATA columns, at least one DATA column,
    ...
    :meth:`~ockham.data_store.DataStore.load_result`.
    """
