...
class TestConnectorsToLlm:
    def test_no_framework_authored_usage_guide(self) -> None:
        # Pure serialization: usage prose, binding names, and "how to call
        # these" framing are the host's responsibility.
        text = _fake_connectors().to_llm()
        assert "# Data connectors" not in text
        assert "await client" not in text
    ...
    def test_empty_collection_is_empty_string(self) -> None:
        # The host decides what to render when the catalog is empty.
